...
public class EventClusterRenderer<T extends ClusterItem> implements ClusterRenderer<T> {
    private static final boolean SHOULD_ANIMATE = Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB;
    private final GoogleMap mMap;
    private final IconGenerator mIconCircleGenerator;
    private final IconGenerator mIconRectGenerator;
    ...
    private ShapeDrawable mColoredRectangleBackground;
    ...
    public EventClusterRenderer(final Context context, GoogleMap map, ClusterManager<T> clusterManager, Bitmap defaultMarkerImage) {
        ...
        mIconCircleGenerator = new IconGenerator(context);
        mIconCircleGenerator.setContentView(makeSquareTextView(context));
        mIconCircleGenerator.setTextAppearance(R.style.ClusterIcon_TextAppearance);
        mIconCircleGenerator.setBackground(makeClusterCircularBackground());

        mIconRectGenerator = new IconGenerator(context);
        mIconRectGenerator.setContentView(makeSquareTextView(context));
        mIconRectGenerator.setTextAppearance(R.style.ClusterIcon_TextAppearance);
    ...
    private LayerDrawable makeClusterCircularBackground() {
    ...
    private LayerDrawable makeClusterRectangularBackground() {
        mColoredRectangleBackground = new ShapeDrawable(new RectShape());
        ShapeDrawable outline = new ShapeDrawable(new RectShape());
        outline.getPaint().setColor(0x80ffffff); // Transparent white.
        LayerDrawable background = new LayerDrawable(new Drawable[]{outline, mColoredRectangleBackground});
        int strokeWidth = (int) (mDensity * 3);
        background.setLayerInset(1, strokeWidth, strokeWidth, strokeWidth, strokeWidth);
        return background;
    }
    ...
    /**
     * Transforms the current view (represented by EventClusterRenderer.mClusters and EventClusterRenderer.mZoom) to a
    ...
    private class RenderTask implements Runnable {
        ...
        @SuppressLint("NewApi")
        public void run() {
            if (clusters.equals(EventClusterRenderer.this.mClusters)) {
                mCallback.run();
                return;
            }
            ...
            if (EventClusterRenderer.this.mClusters != null && SHOULD_ANIMATE) {
                existingClustersOnScreen = new ArrayList<Point>();
                for (Cluster<T> c : EventClusterRenderer.this.mClusters) {
            ...
            mMarkers = newMarkers;
            EventClusterRenderer.this.mClusters = clusters;
            mZoom = zoom;
     ...
     */
    protected void onBeforeClusterItemRendered(T eventItem, MarkerOptions markerOptions) {
        LastFmEvent event = ((EventClusterItem) eventItem).getEvent();

        markerOptions.title(event.getTitle());

        try {
            Bitmap image = CouchbaseManager.retrieveImage(event, LastFmImage.Size.MEDIUM, mHandler);

            if (image == null) {
                image = mDefaultMarkerImage;
            }
    ...
    private boolean isClusteredToPoint(Cluster<T> cluster) {
        Collection<ClusterItem> items = (Collection<ClusterItem>) cluster.getItems();
        LatLng lastPosition = null;
        float[] results = new float[1];

        for (ClusterItem item : items) {
            if (lastPosition != null) {
                Location.distanceBetween(lastPosition.latitude, lastPosition.longitude, item.getPosition().latitude, item.getPosition().longitude, results);
    ...
    protected void onBeforeClusterRendered(Cluster<T> cluster, MarkerOptions markerOptions) {
        ...
        if (descriptor == null) {
            if (isClusteredToPoint(cluster)) {
                mColoredRectangleBackground.getPaint().setColor(getColor(bucket));
                descriptor = BitmapDescriptorFactory.fromBitmap(mIconRectGenerator.makeIcon(getClusterText(bucket)));
            }
            else {
                mColoredCircleBackground.getPaint().setColor(getColor(bucket));
                descriptor = BitmapDescriptorFactory.fromBitmap(mIconCircleGenerator.makeIcon(getClusterText(bucket)));
            }
